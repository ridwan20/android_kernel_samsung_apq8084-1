...
struct max77804k_haptic_data *max77804k_g_hap_data;
...
void max77804k_vibrator_en(bool en)
{
	...
	if (en) {
		if (max77804k_g_hap_data->running)
			return;

		max77804k_haptic_i2c(max77804k_g_hap_data, true);

		max77804k_g_hap_data->running = true;
	} else {
		if (!max77804k_g_hap_data->running)
			return;

		max77804k_haptic_i2c(max77804k_g_hap_data, false);
...
static void max77804k_haptic_shutdown(struct device *dev)
{
	...
	ret = max77804k_update_reg(data->i2c, MAX77804K_HAPTIC_REG_CONFIG2,
				   0x0, MAX77804K_MOTOR_EN);
	if (ret < 0) {
